...
- New `imap-mcp-server/smtp` export subpath, structurally isolated from mailbox-reading and credential-storage code ("The connector knows its limits — Phase 1: yarpost can send without being able to read"). `SmtpService` (plus the `ImapAccount`, `SmtpConfig`, `EmailComposer`, `EmailAttachment` types) is now published from a dedicated barrel, `src/smtp.ts`, built by its own esbuild entry point (`dist/smtp.js`, with `dist/smtp.d.ts` type declarations via `tsc --emitDeclarationOnly`). A send-only consumer — e.g. a shared mail-egress broker that must hold a send credential and nothing else — can `import { SmtpService } from 'imap-mcp-server/smtp'` with no path by which `ImapService` (full mailbox read), `AccountManager` (the local AES-256 credential store), or the MCP tool/web layers can enter its import graph. This is enforced structurally, not by configuration: `tests/smtp-export-isolation.test.ts` builds `src/smtp.ts` via the esbuild API and asserts on the resulting metafile *and* independently walks the raw source import graph (regex-based, since this repo's TypeScript 7 package no longer ships the JS Compiler API), because esbuild silently elides an unused-but-leaked import from the bundle/metafile entirely — a bundle-only check would have missed exactly that case. `package.json` gains an `exports` map (`"."` preserves today's `main`/`dist/index.js` behavior unchanged; `"./smtp"` is the new subpath); deep `imap-mcp-server/dist/...` imports that resolved implicitly before are no longer part of the published surface. Additive and non-breaking. A candidate to upstream to `nikolausm/imap-mcp-server` later — not opened here, left to Rob's call.

### Fixed
- `npm install github:.../imap-mcp-server` (or any git-URL dependency) installed a package with **no `dist/` at all** — not `dist/smtp.js`, not even `dist/index.js` — because `dist/` is gitignored and the package had no `prepare` script to build it after a git checkout (`prepublishOnly` only fires on `npm publish`, never on a git install). Confirmed by actually running `npm install github:robb0995/imap-mcp-server` in a scratch project and hitting `ERR_MODULE_NOT_FOUND` on both `imap-mcp-server` and `imap-mcp-server/smtp` — the acceptance criterion for the `./smtp` subpath ("consumable as a git dependency") wasn't actually met. Added `"prepare": "node build.mjs"`, npm's documented hook for building git dependencies on install (it temporarily installs `devDependencies` to run it, which is why `esbuild` and `typescript` are devDependencies here). No effect on registry installs, which already ship a prebuilt `dist/`.
